...
import { C, fmt, fmtShort, CATEGORIES, MONTHS, ACCOUNT_ICONS, ACCOUNT_TYPES, BADGES, genId, filterTxns, filterTxnsByMonthAndPeriod, getAvailableMonths, calcIncome, calcExpenses, DEFAULT_THEME, applyTheme } from "../utils/constants.js";
...
export function TransactionsPage({ txns=[], accounts=[], onAdd, onEdit, onDelete }) {
  ...
  const [selectedMonth, setSelectedMonth] = useState({ year:now.getFullYear(), month:now.getMonth(), label:MONTHS[now.getMonth()], yearShort:now.getFullYear().toString().slice(2), isCurrent:true });
  const [confirm, setConfirm] = useState(null);
  const [editing, setEditing] = useState(null);
  const [showForm, setShowForm] = useState(false);

  const filtered = useMemo(() => {
    let t = filterTxnsByMonthAndPeriod(txns, selectedMonth.year, selectedMonth.month, "monthly");
  ...
  }, [txns, selectedMonth, filterType, filterCat, search]);

  const income = calcIncome(filtered);
  const expenses = calcExpenses(filtered);
  const transfers = filtered.filter(t=>t.type==="transfer").reduce((s,t)=>s+(parseFloat(t.amount)||0),0);

  function getAcctName(id) { return accounts.find(a=>a.id===id)?.name || "—"; }
  const periodLabel = `${selectedMonth.label} ${selectedMonth.year}`;
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:14 }}>
        <div style={{ fontSize:11, color:C.textMid, fontWeight:600, textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>📅 Month</div>
        <MonthTabs selected={selectedMonth} onChange={setSelectedMonth} />
      </div>

      {/* Summary KPIs */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))", gap:8 }}>
        {[
          ["Income", income, C.green, "💚"],
          ["Expenses", expenses, C.red, "🔴"],
          ["Net", income-expenses, income-expenses>=0?C.green:C.red, "📊"],
          ["Transfers", transfers, C.cyan, "🔄"],
        ].map(([l,v,c,icon])=>(
          <div key={l} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 12px" }}>
            <div style={{ fontSize:10, color:C.textMid, marginBottom:3 }}>{icon} {l}</div>
            <div style={{ fontSize:14, fontWeight:700, color:c }}>{v>=0?fmt(v,0):"-"+fmt(Math.abs(v),0)}</div>
          </div>
        ))}
      </div>

      {/* Filter bar */}
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:12, display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:4 }}>
          {[["All","All",C.textMid],["income","💚 Income",C.green],["expense","🔴 Expense",C.red],["transfer","🔄 Transfer",C.cyan]].map(([val,label,col])=>(
            <button key={val} onClick={()=>setFilterType(val)} style={{
              padding:"5px 10px", borderRadius:20, border:"none", cursor:"pointer", fontSize:11,
              background: filterType===val ? col+"33" : C.surface,
              color: filterType===val ? col : C.textMid,
              fontWeight: filterType===val ? 700 : 400,
              outline: filterType===val ? `1px solid ${col}` : "none"
            }}>{label}</button>
          ))}
        </div>
        <select value={filterCat} onChange={e=>setFilterCat(e.target.value)} style={{ padding:"6px 10px", background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, color:C.text, fontSize:12, outline:"none" }}>
          {["All",...CATEGORIES].map(v=><option key={v} value={v}>{v==="All"?"All Categories":v}</option>)}
        </select>
        <Btn onClick={()=>setShowForm(true)}>+ Add</Btn>
      </div>

      {/* Transaction count */}
      <div style={{ fontSize:11, color:C.textDim, paddingLeft:2 }}>
        {filtered.length} transaction{filtered.length!==1?"s":""} in {periodLabel}
        {(filterType!=="All"||filterCat!=="All"||search) ? " (filtered)" : ""}
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
        {filtered.length === 0
          ? <div style={{ padding:32, textAlign:"center", color:C.textDim }}>
              ...
              {(filterType!=="All"||filterCat!=="All"||search) && <div style={{ fontSize:11, marginTop:6 }}>Try clearing your filters</div>}
            </div>
          : filtered.map(t => (
              <div key={t.id} style={{ display:"flex", alignItems:"center", padding:"12px 16px", borderBottom:`1px solid ${C.border}`, gap:10 }}>
                <div style={{ width:32, height:32, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, flexShrink:0,
                  background: t.type==="income" ? C.green+"22" : t.type==="transfer" ? C.cyan+"22" : C.red+"22"
                }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, color:C.textMid }}>
                    {t.type==="transfer"?"Transfer":t.category} · {getAcctName(t.accountId)}
                    {t.type==="transfer"&&t.toAccountId ? ` → ${getAcctName(t.toAccountId)}` : ""} · {t.date}
                  </div>
                </div>
                <div style={{ fontSize:14, fontWeight:700, color:t.type==="income"?C.green:t.type==="transfer"?C.cyan:C.red, minWidth:72, textAlign:"right" }}>
                  {t.type==="income"?"+":t.type==="transfer"?"↔":"-"}{fmt(t.amount,0)}
                </div>
                <div style={{ display:"flex", gap:4 }}>
...
function TxnFormInline({ accounts=[], initial=null, onSave, onClose }) {
  ...
  const [form, setForm] = useState(initial||{ type:"expense", amount:"", category:"Food", description:"", date:today, accountId:accounts[0]?.id||"", toAccountId:"" });
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
      <div style={{ display:"flex", gap:3, background:C.surface, padding:3, borderRadius:8, marginBottom:10 }}>
        {[["income","💚 Income",C.green],["expense","🔴 Expense",C.red],["transfer","🔄 Transfer",C.cyan]].map(([v,label,color])=>(
          <button key={v} onClick={()=>set("type",v)} style={{ flex:1, padding:"7px", border:"none", borderRadius:6, cursor:"pointer",
            background:form.type===v?color:"transparent",
            color:form.type===v?"#fff":C.textMid, fontWeight:form.type===v?600:400, fontSize:11
          }}>{label}</button>
        ))}
      </div>
      {form.type==="transfer"&&<div style={{background:C.cyan+"18",border:`1px solid ${C.cyan}33`,borderRadius:8,padding:"6px 10px",marginBottom:6,fontSize:11,color:C.cyan}}>Transfers are excluded from income/expense totals.</div>}
      ...
      <Select label={form.type==="transfer"?"From Account":"Account"} value={form.accountId} onChange={e=>set("accountId",e.target.value)} options={accounts.map(a=>({value:a.id,label:`${a.icon} ${a.name}`}))} />
      {form.type==="transfer"&&<Select label="To Account" value={form.toAccountId} onChange={e=>set("toAccountId",e.target.value)} options={[{value:"",label:"Select account..."},...accounts.filter(a=>a.id!==form.accountId).map(a=>({value:a.id,label:`${a.icon} ${a.name}`}))]} />}
      ...
      <Row style={{ marginTop:8 }}>
        ...
        <button onClick={save} style={{ flex:1, padding:"9px", background:form.type==="transfer"?C.cyan:C.accent, border:"none", borderRadius:8, color:"#fff", cursor:"pointer", fontWeight:600 }}>{initial?"Update":"Add"}</button>
      </Row>
    </div>
  );
}
...
export function BudgetPage({ budget, txns=[], onSaveBudget }) {
  ...
  const filtered = filterTxnsByMonthAndPeriod(txns, selectedMonth.year, selectedMonth.month, "monthly");
  const catActuals = {};
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:14 }}>
        <div style={{ fontSize:11, color:C.textMid, fontWeight:600, textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>📅 Viewing Month</div>
        <MonthTabs selected={selectedMonth} onChange={setSelectedMonth} />
      </div>
