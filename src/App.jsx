...
import {
  C, applyTheme, DEFAULT_THEME, genId,
  defaultBudget
...
function clearCache(uid) {
  ["txns","accounts","goals","debts","investments","nwHistory","budget","theme"]
    .forEach(k => { try { localStorage.removeItem(cacheKey(uid, k)); } catch(e) {} });
}
...
export default function App() {
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, u => {
      ...
      if (!u) {
        setLoading(false);
        setTxns([]); setAccounts([]); setGoals([]);
        setDebts([]); setInvestments([]); setNwHistory([]);
  ...
  async function editTxn(t) {
    ...
    if (old && old.accountId) {
      ...
      if (acct) {
        ...
        await fbSet(uid, "accounts", acct.id, { ...acct, balance: (parseFloat(acct.balance)||0) + rev + nd });
      }
    }
    await fbSet(uid, "txns", t.id, t);
  }
